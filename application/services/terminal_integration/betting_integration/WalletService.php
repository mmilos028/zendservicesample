...
require_once SERVICES_DIR . DS . 'terminal_integration' . DS . 'betting_integration' . DS . 'walletUser.php';
require_once SERVICES_DIR . DS . 'terminal_integration' . DS . 'betting_integration' . DS . 'bookType.php';
...
require_once SERVICES_DIR . DS . 'terminal_integration' . DS . 'betting_integration' . DS . 'walletUserApplicationSettings.php';
require_once SERVICES_DIR . DS . 'terminal_integration' . DS . 'betting_integration' . DS . 'WalletException.php';
...
/**
 * 
 * Wallet Service for Betting integration ...
 *
 */
class WalletService{
	...
	*/
	public function GetUserInfoByUserID($userID){
		...
		if(!isset($userID) || strlen($userID) == 0 || intval($userID) <= 0){
			...
			$message = "BETTING INTEGRATION: " . $procedure;
		...
		//$errorHelper->bettingIntegrationAccess($message, $message);
		
		$modelBettingIntegration = new BettingIntegrationModel();
		$user_details = $modelBettingIntegration->getUserInfoByUserID($userID);
	...
	*/
	public function GetUserInfo($sessionID){
		...
		$modelBettingIntegration->checkPendingTransactionsForPlayer($sessionID);
		//
		$user_details = $modelBettingIntegration->getUserInfo($sessionID);
		if($user_details['status'] != OK){
	...
	*/
	public function Book($userID, $amount, $bookType, $allowInsufficientCredit){
		...
		//$errorHelper->bettingIntegrationAccess($message, $message);
		if($bookType == 'INC') $bookType = 'INC_WET';
		if($bookType == 'DEC') $bookType = 'DEC_WET';
		$modelBettingIntegration = new BettingIntegrationModel();
		$result = $modelBettingIntegration->book($userID, $amount, $bookType);
		if($result['status'] != OK){
	...
	*/
	public function RollbackBook($bookID){
		...
		$modelBettingIntegration = new BettingIntegrationModel();
		$result = $modelBettingIntegration->rollbackBook($bookID);
		if($result['status'] != OK){
	 ...
	 * @throws WalletException
	*/
	private function TicketInserted($ticketID, $application, $receiveDateTime, $domain, $bookID){
		...
		$errorHelper = new ErrorHelper();
		$errorHelper->bettingIntegrationAccessLog($procedure);
	}
	 ...
	 * @throws WalletException
	*/ 
	private function RollbackTicketInserted($ticketID, $application, $bookID){
	 ...
	 * @throws WalletException
	*/ 
	private function TicketWon($ticketID, $application, $bookID){
	 ...
	 * @throws WalletException
	*/
	private function RollbackTicketWon($ticketID, $application, $bookID){
	 ...
	 * @throws WalletException
	*/
	private function TicketLost($ticketID, $application){
	 ...
	 * @throws WalletException
	*/ 
	private function RollbackTicketLost($ticketID, $application){

...
class AccountManager {
     ...
     * @return mixed
     */
    public function updatePlayer($backoffice_session_id, $player_id, $email, $banned, $zip, $phone, $address, $birthday, $first_name, $last_name, $city, $country){
		...
		*/
        try {
            ...
            require_once MODELS_DIR . DS . 'terminal_integration' . DS . 'PlayerModel.php';
            $modelPlayer = new PlayerModel();
            return $modelPlayer->manageUser($backoffice_session_id, UPDATE, null, null, null, null, null, $email, $country, null, $banned, $zip, $phone, $address, $birthday, $first_name, $last_name, $city, $player_id, null, null, null, null, null);
        }catch(Zend_Exception $ex){
            $detected_ip_address = IPHelper::getRealIPAddress();
			$errorHelper = new ErrorHelper();
     ...
     * @return mixed
     */
	public function playerDetails($backoffice_session_id, $player_id){
		try{
            if(strlen($backoffice_session_id) == 0 || strlen($player_id) == 0) {
                return array("status" => NOK, "message" => NOK_INVALID_DATA);
            }

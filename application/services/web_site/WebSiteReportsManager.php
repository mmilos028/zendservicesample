...
class WebSiteReportsManager {	
	 ...
	 * @return mixed
	 */
	public function listLimitsNew($session_id){
		if(!$this->isSecureConnection()){
			return array("status"=>NOK, "message"=>NON_SSL_CONNECTION);
		}
		if(!isset($session_id) || strlen($session_id) == 0 || $session_id == "null"){
			return array("status"=>NOK, "result"=>null);
		}
		try{
			$session_id = strip_tags($session_id);
			require_once MODELS_DIR . DS . 'WebSiteModel.php';
			...
			return array("status"=>OK, "result"=>$resArray);
		}catch(Zend_Exception $ex){
			$errorHelper = new ErrorHelper();
	 ...
	 * @return mixed
	 */
	public function listPlayerHistory($session_id, $startdate, $enddate, $pageNo, $perPage, $column, $order){
		if(!$this->isSecureConnection()){
			return NON_SSL_CONNECTION;
		}
		if(!isset($session_id) || strlen($session_id) == 0 || $session_id == "null" || !isset($startdate) || !isset($startdate) || !isset($enddate)){
			return false;
		}		
		if(!isset($pageNo)){
			$pageNo = 1;
        ...
        try{
            ...
            if($pageNo == 1){
                $arrData = $modelReports->listPlayerHistory($player_name, $startdate, $enddate, $pageNo, $perPage, $column, $order);
                if($arrData == false){
                    return false;
                }
            ...
            foreach($arrData[0] as $row){
                ...
                $resArray[] = array(
                    ...
                    "cash_in"=> $row['cash_in'],
                    "cash_out"=>(-1 * $row['cash_out']),
                    "start_credits"=>$row['start_credits'],
                    "end_credits"=>$row['end_credits'],
                    "game_in"=>$row['game_in'],
                    "game_out"=>$row['game_out'],
                    "game_win"=>$row['game_win'],
                    "game_payback"=>$row['game_payback'],
                    "show_details"=>$flagShowDetails
                );
            }
            return array("total_items"=>$total_items, "total_pages"=>$total_pages, "data"=>$resArray, "actual_credits"=>$actual_credits);
        }catch(Zend_Exception $ex){
			$errorHelper = new ErrorHelper();
			$mail_message = "WebSiteReportsManager::ListPlayerHistory(session_id = {$session_id}, startdate = {$startdate}, enddate = {$enddate}, pageNo = {$pageNo}, perPage = {$perPage}, column = {$column}, order = {$order}) Exception: <br /> " . CursorToArrayHelper::getExceptionTraceAsString($ex);
			$log_message = CursorToArrayHelper::getExceptionTraceAsString($ex);
			$errorHelper->siteError($mail_message, $log_message);
			return false;
		}
	...
	* @return mixed
	*/
	public function listPlayerHistoryDetails($session_id, $pageNo, $perPage, $column, $order){
		if(!$this->isSecureConnection()){
			return NON_SSL_CONNECTION;
		}
		if(!isset($session_id) || strlen($session_id) == 0 || $session_id == "null"){
        ...
        try {
            ...
            $modelReports = new ReportsModel();
            if ($pageNo == 1) {
                $arrData = $modelReports->listGameSessionDetails($session_id, $pageNo, $perPage, $column, $order);
                if ($arrData == false) {
                    return false;
                }
            ...
            return array("total_items" => $total_items, "total_pages" => $total_pages, "data" => $resArray,
			...
			$errorHelper = new ErrorHelper();
			$mail_message = "WebSiteReportsManager::listPlayerHistoryDetails(session_id = {$session_id}, pageNo = {$pageNo}, perPage = {$perPage}, column={$column}, order={$order}) Exception: <br /> " . CursorToArrayHelper::getExceptionTraceAsString($ex);
			$log_message = CursorToArrayHelper::getExceptionTraceAsString($ex);
			$errorHelper->siteError($mail_message, $log_message);
			return false;
		}
	...
	* @return mixed
	*/
	public function listPlayerHistorySubdetails($session_id, $pageNo, $perPage, $column, $order){
		if(!$this->isSecureConnection()){
			return NON_SSL_CONNECTION;
		}
		if(!isset($session_id) || strlen($session_id) == 0 || $session_id == "null"){
	...
	* @return mixed
	*/
	public function listCreditTransfers($session_id, $startdate, $enddate, $pageNo, $perPage, $column, $order){
		if(!$this->isSecureConnection()){
			return NON_SSL_CONNECTION;
		}
	...
	* @return mixed
	*/
	public function listAvailableBonusCampaigns($affiliate_username, $player_id){
        /*
        $errorHelper = new ErrorHelper();
        $message = "WebSiteReportsManager::listAvailableBonusCampaigns(affiliate_username = {$affiliate_username}, player_id = {$player_id})";
        ...
        */
        ...
        if(!isset($affiliate_username) || strlen($affiliate_username) == 0 || $affiliate_username == "null"){
            return array("status"=>NOK, "report"=>array());
        }
        ...
        try {
            ...
            if ($arrData['status'] == OK) {
                ...
                return array("status" => OK, "affiliate_username" => $affiliate_username, "player_id" => $player_id, "report" => $report);
            } else {
                return array("status" => NOK, "report" => array());
            }
        }catch(Zend_Exception $ex){
    ...
    /**
	*
	* List player available bonus campaigns
	* @param int $player_id
	* @return mixed
	*/
	public function listPlayerAvailableBonusCampaigns($player_id){
		if(!$this->isSecureConnection()){
			return array("status"=>NOK, "message"=>NON_SSL_CONNECTION);
		}
        $player_id = strip_tags($player_id);
        ...
        try {
            ...
            if ($arrData['status'] == OK) {
                ...
                return array("status" => OK, "player_id" => $player_id, "report" => $report);
            } else {
                return array("status" => NOK, "player_id" => $player_id, "report" => array());
            }
        }catch(Zend_Exception $ex){
			$errorHelper = new ErrorHelper();
    ...
    /**
	*
	* List player active bonus and promotions
	* @param int $player_id
	* @return mixed
	*/
            ...
            if ($arrData['status'] == OK) {
                ...
                foreach ($arrData['cursor'] as $c) {
                    $report[] = array(
                        ...
                        'bonus_promotion' => $c['bonus_promotion'],
     ...
     * @return array
     */
    public function listTopWonJackpots($affiliate_name){
        if(!$this->isSecureConnection()){
			return array("status"=>NOK, "message"=>NON_SSL_CONNECTION);
		}
        ...
        try {
            ...
            if ($arrData['status'] == OK) {
                ...
                return array("status" => OK, "affiliate_name" => $affiliate_name, "report" => $report);
            } else {
                return array("status" => NOK, "affiliate_name" => $affiliate_name, "report" => array());
            }
        }catch(Zend_Exception $ex){
			$errorHelper = new ErrorHelper();
     ...
     * @return array
     */
    public function listCurrentJackpotLevels($affiliate_name, $currency){
        if(!$this->isSecureConnection()){
			return array("status"=>NOK, "message"=>NON_SSL_CONNECTION);
		}
        $affiliate_name = strip_tags($affiliate_name);
     ...
     * @return array
     */
    public function listHighWins($affiliate_name){
        if(!$this->isSecureConnection()){
			return array("status"=>NOK, "message"=>NON_SSL_CONNECTION);
		}
        $affiliate_name = strip_tags($affiliate_name);

	...
	*/
	public function GetUserInfoByUserID($userID){
		...
		if(!isset($userID) || strlen($userID) == 0 || intval($userID) <= 0){
			...
			$message = "DETECTED IP ADDRESS: {$detected_ip_address} <br /> SPORT BET INTEGRATION Error: " . $procedure;
        ...
        if($this->DEBUG_MODE) {
            $errorHelper = new ErrorHelper();
            $message = "DETECTED IP ADDRESS: {$detected_ip_address} <br /> GetUserInfoByUserID(userID = {$userID})";
		...
		if($user_details['status'] != OK){
			...
			$message = "DETECTED IP ADDRESS: {$detected_ip_address} <br /> SPORT BET INTEGRATION: " . $procedure;
			$errorHelper = new ErrorHelper();
			$errorHelper->sportBettingIntegrationError($message, $message);

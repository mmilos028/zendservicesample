	...
	*/
		...
		try{
			...
			if($result['status'] == OK){
                ...
                $json_message = Zend_Json::encode(
                    array("status"=>OK, "result"=>$currency_list)
                );
			    exit($json_message);
			}else{
                $json_message = Zend_Json::encode(array("status"=>NOK, "message"=>NOK_EXCEPTION));
	...
	$bank_account, $bank_country, $swift, $iban, $receive_email = 1, $currency, $ip_address, $registration_code, $tid_code = '', $language = 'en_GB'){
		...
		try{
            ...
            if($bo_session_id == 0){
                $json_message = Zend_Json::encode(
                    array("status"=>NOK, "message"=>NOK_INTERNAL_ERROR, "details"=>"Invalid BO Session")
                );
			    exit($json_message);
            }
            ...
            */
            ...
            if($res['status'] != OK){
                ...
                */
                ...
                if ($res['error_code'] == 20713) {
                    $json_message = Zend_Json::encode(
                        array("status" => NOK, "message" => NOK_WRONG_FNAME_LNAME_BIRTHDAY)
                    );
                    exit($json_message);
                } else if ($res['error_code'] == 20714) {
                    $json_message = Zend_Json::encode(
                        array("status" => NOK, "message" => NOK_WRONG_MAIL)
                    );
                    exit($json_message);
                } else if ($res['error_code'] == 20715) {
                    $json_message = Zend_Json::encode(
                        array("status" => NOK, "message" => NOK_WRONG_FNAME_LNAME_ADDRESS)
                    );
                    exit($json_message);
                } else if ($res['error_code'] == 20721) {
                    $json_message = Zend_Json::encode(
                        array("status" => NOK, "message" => NOK_WRONG_FNAME_LNAME_PHONE)
                    );
                    exit($json_message);
                } else if ($res['error_code'] == 20729) {
                    $json_message = Zend_Json::encode(
                        array("status" => NOK, "message" => NOK_INVALID_REGISTRATION_CODE)
                    );
                    exit($json_message);
                } else {
                    $json_message = Zend_Json::encode(
                        array("status" => NOK, "message" => NOK_INTERNAL_ERROR, "details" => "Unknown error occured")
                    );
                    exit($json_message);
                }
            }
                ...
                */
                ...
                if($validationHash != null){
                    ...
                    if($site_settings['status'] != OK){
                        $json_message = Zend_Json::encode(
                            array("status"=>NOK, "message"=>NOK_INTERNAL_ERROR, "details"=>"Error finding player site settings")
                        );
                        exit($json_message);
                    }
                    ...
                    $json_message = Zend_Json::encode(
                        array("status"=>NOK, "message"=>NOK_INTERNAL_ERROR, "details"=>"Error receiving inserted player validation hash for his activation mail")
                    );
                    exit($json_message);
                }
            }else{
			...
			$errorHelper->siteError($mail_message, $log_message);
            $json_message = Zend_Json::encode(
                array("status"=>NOK, "message"=>NOK_INTERNAL_ERROR, "details"=>"Error inserting player or seting player verification status")
            );
            exit($json_message);
		}
	}
		...
		try{
            ...
            $json_message = Zend_Json::encode(
                array(
                    ...
                    "credit_status"=>NumberHelper::convert_double($details['credit_status']),
                    "credit_status_formatted"=>NumberHelper::format_double($details['credit_status']),
                    "total_credits"=>NumberHelper::convert_double($details['credit_status']),
                    "total_credits_formatted"=>NumberHelper::format_double($details['credit_status']),
                    "credits"=>NumberHelper::convert_double($details['credits']),
                    "credits_formatted"=>NumberHelper::format_double($details['credits']),
                    "free_credits"=>NumberHelper::convert_double($details['credits']),
                    "free_credits_formatted"=>NumberHelper::format_double($details['credits']),
                    "credits_restricted"=>NumberHelper::convert_double($details['credits_restricted']),
                    "credits_restricted_formatted"=>NumberHelper::format_double($details['credits_restricted']),
                    "bonus_restricted"=>NumberHelper::convert_double($details['bonus_restricted']),
                    "bonus_restricted_formatted"=>NumberHelper::format_double($details['bonus_restricted']),
                    "bonus_win_restricted"=>NumberHelper::convert_double($details['bonus_win_restricted']),
                    "bonus_win_restricted_formatted"=>NumberHelper::format_double($details['bonus_win_restricted']),
                    "promotion"=>NumberHelper::convert_double($details['promotion']),
                    "promotion_formatted"=>NumberHelper::format_double($details['promotion']),
	...
	$phone_number = "", $bank_account, $bank_country, $swift, $iban, $receive_email = 1, $ip_address, $language = "en_GB"){
		...
		try{
			...
			try{
				...
				$errorHelper->siteError($mail_message, $log_message);
                $json_message = Zend_Json::encode(
                    array("status"=>NOK, "message"=>NOK_INTERNAL_ERROR, "details"=>"Error updating player information")
                );
			    exit($json_message);
			}
			$modelAuthorization->closeBoSession($bo_session_id);
	 ...
	 * @return mixed
	 */
	public static function resetPassword($session_id, $password_old, $password_new, $ip_address){
		...
		try{
			...
			if($res == 1){
                $json_message = Zend_Json::encode(array("status"=>OK, "status_out"=>OK));
			    exit($json_message);
			}
			else{
				$errorHelper = new ErrorHelper();
				...
				$errorHelper->siteError($mail_message, $log_message);
				$json_message = Zend_Json::encode(array("status"=>OK, "status_out"=>NOK));
			    exit($json_message);
			}
		}catch(Zend_Exception $ex){
			$errorHelper = new ErrorHelper();
	...
	public static function validatePlayer($username){
		try{
            ...
            if($exist == YES){
                $json_message = Zend_Json::encode(array("status"=>OK, "status_out"=>YES));
			    exit($json_message);
            }else {
                $json_message = Zend_Json::encode(array("status"=>OK, "status_out"=>$exist));
			    exit($json_message);
            }
		}catch(Zend_Exception $ex){
			$errorHelper = new ErrorHelper();
	 ...
	 * @return mixed
	 */
	public static function validatePlayerPhone($player_id, $player_phone, $white_label_name){
		try{
			if(strlen($player_phone) == 0 || strlen($player_id) == 0 || strlen($white_label_name)){
                $json_message = Zend_Json::encode(array("status"=>OK, "message"=>NOK_INVALID_DATA));
			    exit($json_message);
			}
            ...
            if($result['status'] == OK){
                $json_message = Zend_Json::encode(array("status"=>OK, "phone_available"=>$result['phone_available']));
			    exit($json_message);
            }else{
	 ...
	 * @return mixed
	 */
	public static function validatePlayerEmail($player_id, $player_email, $white_label_name){
		try{
			...
			if($result['status'] == OK){
                $json_message = Zend_Json::encode(array("status"=>OK, "email_available"=>$result['email_available']));
			    exit($json_message);
            }else{
                $json_message = Zend_Json::encode(array("status"=>NOK, "message"=>$result['message']));
			    exit($json_message);
	 ...
	 * @return mixed
	 */
	public static function validatePlayerFirstNameLastNameBirthday($player_id, $player_first_name, $player_last_name, $player_birthday, $white_label_name){
		try{
			...
			if($result['status'] == OK){
                $json_message = Zend_Json::encode(array("status"=>OK, "combination_available"=>$result['combination_available']));
			    exit($json_message);
            }else{
                $json_message = Zend_Json::encode(array("status"=>NOK, "message"=>$result['message']));
			    exit($json_message);
	 ...
	 * @return mixed
	 */
	public static function validatePlayerFirstNameLastNameAddress($player_id, $player_first_name, $player_last_name, $player_city, $player_address, $player_address2, $white_label_name){
		try{
	 ...
	 * @return mixed
	 */
	public static function validatePlayerFirstNameLastNamePhone($player_id, $player_first_name, $player_last_name, $player_phone, $white_label_name){
		try{
	 ...
	 * @return mixed
	 */
	public static function playerCredits($session_id, $ip_address){
		if(!isset($session_id) || !isset($ip_address)){
            $json_message = Zend_Json::encode(array("status"=>NOK, "message"=>NOK_INVALID_DATA));
			exit($json_message);
		}
		try{
            ...
            $json_message = Zend_Json::encode(array(
                ...
                "credit_status"=> NumberHelper::convert_double($result['credits']),
                "credit_status_formatted"=> NumberHelper::format_double($result['credits']),
            ));
            exit($json_message);
		}catch(Zend_Exception $ex){
			$errorHelper = new ErrorHelper();

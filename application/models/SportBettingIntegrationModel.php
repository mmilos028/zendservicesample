...
class SportBettingIntegrationModel{
     ...
     */
    public function checkGGLGameStatus($session_id){
        /* @var $dbAdapter Zend_Db_Adapter_Oracle */
        $dbAdapter = Zend_Registry::get('db_auth');
        $dbAdapter->beginTransaction();
        try{
            $stmt = $dbAdapter->prepare('CALL BETKIOSK.CHECK_GGL_GAME_ON_OFF(:p_session_id_in, :p_status_out)');
            ...
            $message = "GGL INTEGRATION Error <br /> BETKIOSK.CHECK_GGL_GAME_ON_OFF(:p_session_id_in = {$session_id}, :p_status_out =) <br /> Detected IP Address = {$detected_ip_address} <br /> Exception Error: <br />" . CursorToArrayHelper::getExceptionTraceAsString($ex);
     ...
     * @throws Zend_Exception
     */
    public function checkBetkioskGameStatus($session_id){
        /* @var $dbAdapter Zend_Db_Adapter_Oracle */
        $dbAdapter = Zend_Registry::get('db_auth');
        $dbAdapter->beginTransaction();
        try{
            $stmt = $dbAdapter->prepare('CALL BETKIOSK.CHECK_BETKIOSK_GAME_ON_OFF(:p_session_id_in, :p_status_out)');
            $stmt->bindParam(':p_session_id_in', $session_id);
            $status_out = "";
            $stmt->bindParam(':p_status_out', $status_out, SQLT_CHR, 255);
            $stmt->execute();
            ...
            $detected_ip_address = IPHelper::getRealIPAddress();
            $errorHelper = new ErrorHelper();
            $message = "SPORT BET INTEGRATION Error <br /> BETKIOSK.CHECK_BETKIOSK_GAME_ON_OFF(:p_session_id_in = {$session_id}, :p_status_out =) <br /> Detected IP Address = {$detected_ip_address} <br /> Exception Error: <br />" . CursorToArrayHelper::getExceptionTraceAsString($ex);
	...
	*/
    ...
    public function book($user_id, $amount, $book_type){
        ...
        try{
            ...
            /*
            $errorHelper = new ErrorHelper();
            $message = "<br /> BETKIOSK.BOOK(userID = {$user_id}, amount = {$amount}, BookType = {$book_type}, :BookID = {$book_id}, :INSUFFICIENT_CREDIT = {$insufficient_credit}, :UNEXPECTED_ERROR = {$unexpected_error}, :UNKNOWN_USERID = {$unknown_userid}, :WRONG_BOOK_TYPE = {$wrong_book_type})";
